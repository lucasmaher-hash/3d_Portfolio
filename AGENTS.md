...
- ⏳ **Not yet reviewed on a real device by Codex** — everything was verified over CDP at
...
Two Codex sessions ran in parallel on this day and both are folded in below. **Scope note:** the
...
1. **The PinkRoom wall's rib shading broke, and the cause was a stale `.blend`, not a bad edit.**
   ...
   Codex session re-exported for unrelated BlueRoom work and shipped the regression. Two distinct
...
3. **Two Codex sessions on one Blender instance is a real hazard, and item 1 is what it looks
...
different Codex session re-exported hours later for unrelated work and the wall lost all its
